...
const { CLValueBuilder, RuntimeArgs, CLPublicKey, CLString, CLByteArray } = require("casper-js-sdk");
...
const CEP78 = class {
    ...
    async approveForAll({keys, owner,  operator, paymentAmount, ttl}) {
        // let key = createRecipientAddress(operator);
        let key = operator;
        let this_owner = createRecipientAddress(CLPublicKey.fromHex(owner))
        let runtimeArgs = RuntimeArgs.fromMap({
            token_owner: this_owner,
            operator: key,
    ...
    async transfer({keys, source, recipient, tokenId, paymentAmount, ttl}) {
        ...
        if (identifierMode == 0) {
            runtimeArgs = RuntimeArgs.fromMap({
                ...
                source_key: createRecipientAddress(CLPublicKey.fromHex(source)),
    ...
    async registerOwnerForContract({ keys, contract, paymentAmount, ttl }) {
        // Owner input is accountHash
        // tokenOwner = tokenOwner.startsWith("account-hash-")
        //     ? tokenOwner.slice(13)
        //     : tokenOwner;
        ...
        let nftContractHash = new CLByteArray(Uint8Array.from(Buffer.from(contract, "hex")))


        const contractOwnerKey = createRecipientAddress(nftContractHash)


        // let token_metadata = new CLString(JSON.stringify(metadataJson))
        let runtimeArgs = {};
        runtimeArgs = RuntimeArgs.fromMap({
            token_owner: contractOwnerKey,
            // token_meta_data: token_metadata,
        });
